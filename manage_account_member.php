    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        if (password_verify($password_old, $current_password)) {
            ...
            if ($password_new === $password_con) {
                ...
                $update_members_sql = "UPDATE members SET password = '$hashed_password' WHERE member_id = '$member_id'";
                $update_users_sql = "UPDATE users SET password = '$hashed_password' WHERE member_id = '$member_id'";

                if (mysqli_query($connection, $update_members_sql) && mysqli_query($connection, $update_users_sql)) {
...
if (isset($_POST['upd_name'])) {
    ...
    $update_members_sql = "UPDATE members SET username = '$new_username' WHERE member_id = '$member_id'";
    $update_users_sql = "UPDATE users SET username = '$new_username' WHERE member_id = '$member_id'";
